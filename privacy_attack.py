...
# we vary samples from 300, 3,000, 30,000
samples = int(30000)

# to calculate estimation time
...
for iter in range(0,100):
  ...
  # split churn into train and adversary of equal size
  ...
  # step 4
  density_train_new = kde_train.score_samples(evaluation_outside_training) # score eval_outside examples on train density
  density_adversary_new = kde_adversary.score_samples(evaluation_outside_training) # score eval_outside examples on adversary density
...
print(elapsed_time) # print elapsed time

# save results to csv
np.savetxt("epsilons_real_30000.csv", epsilons, delimiter=",")
np.savetxt("MAPD_real_30000.csv", MAPD_col, delimiter=",")
np.savetxt("MAE_real_30000.csv", MAE_col, delimiter=",")
np.savetxt("MSE_real_30000.csv", MSE_col, delimiter=",")
...
samples = 30000  # we vary samples from 300, 3,000, and 30,000
...
for iter in range(0,100):
  ...
  # utility
  MAPD_train, MAE_train, MSE_train = utility(real_data = train, protected_data = swap25_train)
  MAPD_adv, MAE_adv, MSE_adv = utility(real_data = train, protected_data = swap25_adversary_training)
  MAPD = (MAPD_train+MAPD_adv)/2
  MAE = (MAE_train+MAE_adv)/2
  MSE = (MSE_train+MSE_adv)/2
  MAPD_col = np.append(MAPD_col, MAPD)
  MAE_col = np.append(MAE_col, MAE)
  MSE_col = np.append(MSE_col, MSE)
  print("MAPD = " + str(MAPD))
...
np.savetxt("epsilons_swap25_30000.csv", epsilons, delimiter=",")
np.savetxt("MAPD_swap25_30000.csv", MAPD_col, delimiter=",")
np.savetxt("MAE_swap25_30000.csv", MAE_col, delimiter=",")
np.savetxt("MSE_swap25_30000.csv", MSE_col, delimiter=",")
...
samples = 30000 # we vary between 300, 3,000, 30,000.
...
for iter in range(0,100):
  ...
  swap50_train = swapping(percent = 0.5, data = train)
  swap50_adversary_training = swapping(percent = 0.5, data = adversary_training)

  # stap 1, 2
  params = {"bandwidth": np.logspace(-1, 1, 20)}
  grid_train = GridSearchCV(KernelDensity(), params, n_jobs = -1)
  grid_train.fit(swap50_train)
  kde_train = grid_train.best_estimator_

  grid = GridSearchCV(KernelDensity(), params, n_jobs = -1)
  grid.fit(swap50_adversary_training)
  kde_adversary = grid.best_estimator_
  ...
  # utility
  MAPD_train, MAE_train, MSE_train = utility(real_data = train, protected_data = swap50_train)
  MAPD_adv, MAE_adv, MSE_adv = utility(real_data = train, protected_data = swap50_adversary_training)
  MAPD = (MAPD_train+MAPD_adv)/2
  MAE = (MAE_train+MAE_adv)/2
  MSE = (MSE_train+MSE_adv)/2
  MAPD_col = np.append(MAPD_col, MAPD)
...
np.savetxt("epsilons_swap50_30000.csv", epsilons, delimiter=",")
np.savetxt("MAPD_swap50_30000.csv", MAPD_col, delimiter=",")
np.savetxt("MAE_swap50_30000.csv", MAE_col, delimiter=",")
np.savetxt("MSE_swap50_30000.csv", MSE_col, delimiter=",")

"""# Now we apply the same attack to a copula model.
...
samples = 30000 # we vary this 300, 3,000 and 30,000
output_samples = int(3000) # we keep this 300 for n = 300, otherwise 3,000
...
for iter in range(0,100):
  ...
  train = scaler0.transform(train)
  train = pd.DataFrame(train)
  train.columns = churn.columns

  # scale adversary
  scaler1 = MinMaxScaler(feature_range= (-1, 1))
  scaler1 = scaler1.fit(adversary_training)
  adversary_training = scaler1.transform(adversary_training)
  adversary_training = pd.DataFrame(adversary_training)
  adversary_training.columns = churn.columns

  # train copula for train data
  dist = GaussianMultivariate(distribution=GaussianUnivariate)
  dist.fit(train)
  train_copula = dist.sample(output_samples)
  train_copula = pd.DataFrame(scaler0.inverse_transform(train_copula))
  train_copula.columns = train.columns

  # train copula for adversary data
  dist_adv = GaussianMultivariate(distribution = GaussianUnivariate)
  dist_adv.fit(adversary_training)
  adversary_training_copula = dist_adv.sample(output_samples)
  adversary_training_copula = pd.DataFrame(scaler1.inverse_transform(adversary_training_copula))
  adversary_training_copula.columns = adversary_training.columns

  # step 1, 2
  params = {"bandwidth": np.logspace(-1, 1, 20)}
  grid_train = GridSearchCV(KernelDensity(), params, n_jobs = -1)
  grid_train.fit(train_copula)
  kde_train = grid_train.best_estimator_

  grid = GridSearchCV(KernelDensity(), params, n_jobs = -1)
  grid.fit(adversary_training_copula)
  kde_adversary = grid.best_estimator_
  ...
  # we get an error when a naive model is proposed by the attack. This means TPR = 1 and FPR very close to 1.
  # we then set the privacy risk to zero.
  try:
    epsilons = np.append(epsilons,max(math.log((1 - (1/N) - FPR)/FNR), math.log((1 - (1/N) - FNR)/FPR)))
    print("empirical epsilon = " + str(max(math.log((1 - (1/N) - FPR)/FNR), math.log((1 - (1/N) - FNR)/FPR))))
  except:
  ...
  # utility
  MAPD_train, MAE_train, MSE_train = utility(real_data = train, protected_data = train_copula)
  MAPD_adv, MAE_adv, MSE_adv = utility(real_data = train, protected_data = adversary_training_copula)
  MAPD = (MAPD_train+MAPD_adv)/2
  MAE = (MAE_train+MAE_adv)/2
  MSE = (MSE_train+MSE_adv)/2
  MAPD_col = np.append(MAPD_col, MAPD)
...
np.savetxt("epsilons_copula_30000.csv", epsilons, delimiter=",")
np.savetxt("MAPD_copula_30000.csv", MAPD_col, delimiter=",")
np.savetxt("MAE_copula_30000.csv", MAE_col, delimiter=",")
np.savetxt("MSE_copula_30000.csv", MSE_col, delimiter=",")

"""# GAN without differential privacy"""

# first we create a GAN architecture. This is the same architecture used in the churn application (see paper).

class GAN():
    def __init__(self, privacy):
      self.img_rows = 1 # row = 1 with 1 customer
      self.img_cols = 9 # 9 columns in data
      self.img_shape = (self.img_cols,) # shape of 'image'
      self.latent_dim = (9) # latent dim = number of columns
    ...
    def train(self, data, iterations, batch_size, sample_interval, model_name, generator_losses = [], discriminator_acc = [], correlations = [], accuracy = [], MAPD_collect = [],MSE_collect = [], MAE_collect = []):
      ...
      for epoch in range(iterations):
            ...
            # Select a random batch of customers
...
## in the churn application we had the following optimal settings (see appendix of churn application):
N = 1262423
samples = N
iterations = (100000)+1
batch_size = 128
...
# keep the number of epochs the same
N = 10000
samples = N*3
iterations = 1000
batch_size = 100
...
for iter in range(0,100):
  ...
  # scale data to improve training
  scaler0 = MinMaxScaler(feature_range= (-1, 1))
  scaler0 = scaler0.fit(train)
  ...
  gan_train.train(data = np.array(train_GAN), iterations=iterations, batch_size=batch_size, sample_interval=1000, model_name = "train_without.h5")

  # Generate a batch of new customers
  generator = load_model('train_without.h5') # load trained model
  ...
  gan_adv.train(data = np.array(adversary_training_GAN), iterations=iterations, batch_size=batch_size, sample_interval=1000, model_name = "adversary_without.h5")

  # Generate a batch of new images
  generator = load_model('adversary_without.h5')
  noise = np.random.normal(0, 1, (output_samples, 9))
  gen_imgs = generator.predict(noise)
  ...
  kde_train = grid_train.best_estimator_
  print("best bandwidth for train: {0}".format(grid_train.best_estimator_.bandwidth))

  grid = GridSearchCV(KernelDensity(), params, n_jobs = -1)
  grid.fit(adversary_training_GAN)
  kde_adversary = grid.best_estimator_
  print("best bandwidth for adversary: {0}".format(grid.best_estimator_.bandwidth))
...
np.savetxt("epsilons_GAN_30000_1024.csv", epsilons, delimiter=",")
np.savetxt("MAPD_GAN_30000_1024.csv", MAPD_col, delimiter=",")
np.savetxt("MAE_GAN_30000_1024.csv", MAE_col, delimiter=",")
np.savetxt("MSE_GAN_30000_1024.csv", MSE_col, delimiter=",")
...
for s in samples:
  ...
  iterations = int(N/10)
  batch_size = 100
  epochs = int(iterations/(N/batch_size))
  ...
  gan_train = GAN(privacy = False)
  gan_train.train(data = np.array(train_GAN), iterations=iterations, batch_size=batch_size, sample_interval=1000, model_name = "train_without_MLP.h5")

  # Generate a batch of new customers
  generator = load_model('train_without_MLP.h5')
  noise = np.random.normal(0, 1, (N, 9))
  gen_imgs = generator.predict(noise)
  gen_imgs = scaler0.inverse_transform(gen_imgs)
  train_GAN_outcome = pd.DataFrame(gen_imgs.reshape(N, 9))
  train_GAN_outcome.columns = train_GAN.columns.values

  clf = LogisticRegression(multi_class = 'multinomial', random_state=0, n_jobs = -1).fit(train_GAN_outcome, id)
